(async function main() {
  ...
  app.post("/api/login", async (req, res) => {
    ...
    try {
      ...
      // Простая эвристика: если есть @ и точка после @ — считаем это email
      const isEmailLike = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(input);

      if (isEmailLike) {
        searchBy = "email";
        // Приводим к нижнему регистру и убираем пробелы (если в БД так хранится)
  ...
  app.get("/api/assignments/:assignmentId/submissions", authenticateToken, async (req, res) => {
    ...
    try {
      const assignmentCheck = await pool.query("SELECT teacher_id FROM assignments WHERE id = $1", [assignmentId]);
  ...
  app.get("/api/assignments/:assignmentId/students", authenticateToken, async (req, res) => {
    ...
    try {
      const assignmentResult = await pool.query("SELECT class_id FROM assignments WHERE id = $1 AND teacher_id = $2", [
        assignmentId,
        req.user.id,
      ]);
